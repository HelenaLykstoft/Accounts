    ...
    public class RegisterUserValidator : AbstractValidator<RegisterUserRequest>
    {
        public RegisterUserValidator()
        {
            ...
            RuleFor(dto => dto.Username)
                ...
                .When(dto => !string.IsNullOrEmpty(dto.Username)); // Apply validation only if Username is provided
            ...
            RuleFor(dto => dto.Password)
                ...
                .When(dto => !string.IsNullOrEmpty(dto.Password)); // Apply validation only if Password is provided
            ...
            RuleFor(dto => dto.Email)
                ...
                .When(dto => !string.IsNullOrEmpty(dto.Email)); // Apply validation only if Email is provided
            ...
            RuleFor(dto => dto.PhoneNumber)
                ...
                .When(dto => !string.IsNullOrEmpty(dto.PhoneNumber)); // Apply validation only if PhoneNumber is provided

            // Address validation
            RuleFor(dto => dto.StreetNumber)
                .GreaterThan(0).WithMessage("Street number must be greater than 0.")
                .When(dto => dto.StreetNumber > 0); // Apply validation only if StreetNumber is provided

            RuleFor(dto => dto.StreetName)
                .Matches(@"^[A-Za-zæøåÆØÅ\s]+$")
                .WithMessage("Street name must only contain letters.")
                .When(dto => !string.IsNullOrEmpty(dto.StreetName)); // Apply validation only if StreetName is provided

            RuleFor(dto => dto.City)
                .NotEmpty().WithMessage("City name cannot be empty.")
                .When(dto => !string.IsNullOrEmpty(dto.City)); // Apply validation only if City is provided

            RuleFor(dto => dto.PostalCode)
                .InclusiveBetween(1000, 9999).WithMessage("Postal code must be a 4-digit number.")
                .When(dto => dto.PostalCode > 0); // Apply validation only if PostalCode is provided
        }
    }
}
